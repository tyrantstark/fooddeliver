...
Router.post("/", auth, async (req, res) => {
  ...
  (params["MID"] = process.env.MERCHANT_ID),
    ...
    (params["CALLBACK_URL"] = `${process.env.BASE_URL}/api/paytm/success`),
...
Router.post("/success", async (req, res) => {
  ...
  if (y.RESPCODE !== "01") {
    ...
    res.redirect(`${process.env.BASE_URL}/fail`);
  } else {
    ...
    var mailOptions = {
      ...
      html:
        "Hi, <strong>" +
        name +
        "</strong><h2>Order Successfully Placed!</h2><h3><p>Thank you for ordering from Food Eazy</h3> Your order id: " +
        id +
        "</p><p style='text-align: left;'>Regards, Food Eazy</>",
    };
    await transporter.sendMail(mailOptions, async (error, info) => {
      if (error) {
        ...
        res.redirect(`${process.env.BASE_URL}/success?orderid=${body.ORDERID}`);
      }
    });
  }
});
